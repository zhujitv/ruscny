const translations = {
  zh: {
    ...
    emailCenter: '邮件通知', glossaryCenter: '公共术语', qualityCenter: '翻译质量', dataGovernance: '数据治理', settings: '系统配置', searchEmail: '搜索任务或会议 ID', recipients: '收件人', delivery: '发送结果', searchGlossary: '搜索原词、译词或分类', allDirections: '全部方向', globalTermsHint: '公共术语应用于所有用户，用户私人术语优先。', addTerm: '新增术语', direction: '方向', sourceTerm: '原词', targetTerm: '译词', category: '分类', correctionType: '类型', subject: '数据主体', steps: '处理步骤', businessSettings: '业务配置', settingsHint: '业务开关独立保存并保留完整审计记录。', serviceConnections: '服务接入配置', serviceConnectionsHint: '密钥经过加密保存且永不回显；部署环境配置可逐项迁移到这里。', configured: '已配置', notConfigured: '未配置', replaceSecret: '输入新密钥以替换', configurationReason: '请输入配置变更原因（至少 3 个字）', removeConfiguration: '恢复部署配置', removeConfigurationConfirm: '确定删除后台覆盖值并恢复使用部署环境配置吗？', save: '保存', inspectReason: '请输入查看敏感质量内容的原因', confirmCorrection: '确认纠错', rejectCorrection: '拒绝纠错', decisionReason: '请输入审核原因', termPrompt: '请输入：源语言,目标语言,原词,译词,分类',
  },
  ru: {
    ...
    emailCenter: 'Уведомления', glossaryCenter: 'Общий глоссарий', qualityCenter: 'Качество', dataGovernance: 'Управление данными', settings: 'Настройки', searchEmail: 'Задача или ID встречи', recipients: 'Получатели', delivery: 'Доставка', searchGlossary: 'Исходный термин, перевод или категория', allDirections: 'Все направления', globalTermsHint: 'Общие термины применяются ко всем, личные имеют приоритет.', addTerm: 'Добавить', direction: 'Направление', sourceTerm: 'Исходный термин', targetTerm: 'Перевод', category: 'Категория', correctionType: 'Тип', subject: 'Субъект', steps: 'Шаги', businessSettings: 'Бизнес-настройки', settingsHint: 'Бизнес-переключатели сохраняются отдельно и аудируются.', serviceConnections: 'Подключения сервисов', serviceConnectionsHint: 'Секреты шифруются и никогда не отображаются; настройки среды можно переносить по одной.', configured: 'Настроено', notConfigured: 'Не настроено', replaceSecret: 'Введите новый секрет для замены', configurationReason: 'Укажите причину изменения конфигурации', removeConfiguration: 'Вернуть настройку среды', removeConfigurationConfirm: 'Удалить значение из панели и снова использовать настройку среды развертывания?', save: 'Сохранить', inspectReason: 'Укажите причину просмотра содержимого', confirmCorrection: 'Подтвердить', rejectCorrection: 'Отклонить', decisionReason: 'Укажите причину решения', termPrompt: 'Введите: исходный язык,целевой язык,термин,перевод,категория',
  },
};
...
async function loadSettings() { const [data, connections] = await Promise.all([api('/v1/admin/settings'), api('/v1/admin/service-configurations')]); const list = $('#settingsList'); list.replaceChildren(); for (const item of data.items) { const row = element('div', 'setting-row'); row.append(element('strong', '', item.key)); let input; if (typeof item.value === 'boolean') { input = element('select'); for (const value of ['true', 'false']) { const option = element('option', '', value); option.value = value; option.selected = String(item.value) === value; input.append(option); } } else { input = element('input'); input.value = item.value; } input.dataset.settingInput = item.key; input.dataset.version = item.version; row.append(input, actionButton(t('save'), 'save-setting', item.key)); list.append(row); } renderServiceConfigurations(connections.items); }
...
function renderServiceConfigurations(items) { const root = $('#serviceConfigurations'); root.replaceChildren(); const groups = new Map(); for (const item of items) { if (!groups.has(item.category)) groups.set(item.category, []); groups.get(item.category).push(item); } for (const [category, rows] of groups) { const section = element('section', 'service-configuration-group'); section.append(element('h3', '', category.replaceAll('_', ' '))); for (const item of rows) { const row = element('div', 'service-configuration-row'); const identity = element('div', 'service-configuration-identity'); identity.append(element('strong', '', item.label), element('small', '', `${item.key} · ${item.configured ? t('configured') : t('notConfigured')} · ${item.source}`)); let input; if (item.type === 'boolean') { input = element('select'); for (const value of ['true', 'false']) { const option = element('option', '', value); option.value = value; option.selected = String(item.value) === value; input.append(option); } } else { input = element('input'); input.type = item.secret ? 'password' : item.type === 'integer' ? 'number' : 'text'; input.value = item.secret ? '' : (item.value ?? ''); input.placeholder = item.secret && item.configured ? t('replaceSecret') : ''; } input.dataset.serviceConfigurationInput = item.key; input.dataset.version = item.version; const actions = element('div', 'actions'); actions.append(actionButton(t('save'), 'save-service-configuration', item.key)); if (item.source === 'database') actions.append(actionButton(t('removeConfiguration'), 'remove-service-configuration', item.key)); row.append(identity, input, actions); section.append(row); } root.append(section); } }
async function saveServiceConfiguration(key) { const input = document.querySelector(`[data-service-configuration-input="${CSS.escape(key)}"]`); if (!input.value.trim()) return; const reason = prompt(t('configurationReason'))?.trim(); if (!reason) return; const value = input.type === 'number' ? Number(input.value) : input.tagName === 'SELECT' ? input.value === 'true' : input.value; await api(`/v1/admin/service-configurations/${encodeURIComponent(key)}`, { method: 'PATCH', body: { value, expectedVersion: Number(input.dataset.version), reason } }); input.value = ''; toast(t('operationDone')); await loadSettings(); }
async function removeServiceConfiguration(key) { const input = document.querySelector(`[data-service-configuration-input="${CSS.escape(key)}"]`); if (!confirm(t('removeConfigurationConfirm'))) return; const reason = prompt(t('configurationReason'))?.trim(); if (!reason) return; await api(`/v1/admin/service-configurations/${encodeURIComponent(key)}`, { method: 'DELETE', body: { expectedVersion: Number(input.dataset.version), reason } }); toast(t('operationDone')); await loadSettings(); }
...
document.body.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'save-service-configuration') await saveServiceConfiguration(id);
    if (action === 'remove-service-configuration') await removeServiceConfiguration(id);
